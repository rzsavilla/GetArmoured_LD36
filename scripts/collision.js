...
function AABB(x,y,width,height) {
    ...
    this.collision = function(other) {
        if (other instanceof AABB) {
            ...
            if (dist.x >= 0) { clamp.x = Math.min(dist.x,this.getWidth()/2); }
            else if (dist.x < 0) { clamp.x = Math.max(dist.x,-(this.getWidth()/2)); }
            if (dist.y >= 0) { clamp.y = Math.min(dist.y,this.getHeight()/2); }
            else if (dist.y < 0) { clamp.y = Math.max(dist.y,-(this.getHeight()/2)); }
            //Compute distance fromt AABB edge and circle centre
            var diff = dist.vSubtract(clamp);
            //Compute distance between edges
            var distance = diff.magnitude() - other.getRadius();
